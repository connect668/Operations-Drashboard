...
export default function Dashboard() {
    ...
    (async () => {
      try {
        ...
        const { data: prof } = await supabase
          .from("profiles").select("id, full_name, role, company, company_id, facility_number").eq("id", user.id).maybeSingle();
  ...
  // Step 2 only — search real policies instead of placeholder-only flow
  ...
  const searchPolicies = async (searchText, categoryOverride = "") => {
    ...
    try {
      ...
      let query = supabase
        ...
        .select("id, company_id, company, policy_code, title, category, policy_text, version, is_active")
        .eq("is_active", true)
      ...
      if (cleaned) {
        ...
        query = query.or(`title.ilike.%${safe}%,policy_text.ilike.%${safe}%,policy_code.ilike.%${safe}%`);
      }
  ...
  const handleDecisionSubmit = async () => {
    ...
    try {
      const base = {
        user_id: user.id, company: profile?.company || null, company_id: profile?.company_id || null,
        facility_number: profile?.facility_number || null, user_name: profile?.full_name || "Unknown", user_role: profile?.role || "Manager",
  ...
  const handleCoachingSubmit = async () => {
    ...
    try {
      const { error } = await supabase.from("coaching_requests").insert([{
        user_id: user.id, company: profile?.company || null, company_id: profile?.company_id || null,
        facility_number: profile?.facility_number || null, requester_name: profile?.full_name || "Unknown", requester_role: profile?.role || "Manager",
        submitted_by_role: profile?.role || "Manager", visible_to_role: nextRole,
        request_text: coachingText.trim(), status: "open",
      }]);
      if (error) throw error;
      setCoachingText(""); setCoachingMessage("Coaching request submitted.");
    } catch (e) { setCoachingMessage(e.message || "Failed to submit."); }
  ...
  const fetchTeamDecisions = async () => {
    ...
    try {
      let query = supabase.from("decision_logs").select("*")
        .eq("visible_to_role", profile.role).eq("is_read", false).neq("user_id", user?.id || "")
  ...
  const fetchTeamCoachingRequests = async () => {
    ...
    try {
      let query = supabase.from("coaching_requests").select("*")
  ...
  const fetchManagers = async () => {
    ...
    try {
      let query = supabase.from("profiles").select("id, full_name, role, company, company_id, facility_number")
        .eq("role", "Manager").order("full_name");
  ...
  const markDecisionAsRead = async (id, userId) => {
    try {
      ...
      if (!mgr) {
        const { data } = await supabase.from("profiles").select("id, full_name, role, company, company_id, facility_number").eq("id", userId).maybeSingle();
        mgr = data;
      }
                      ...
                      <div
                        ...
                        style={{
                          ...
                          ...(selectedPolicy?.id === policy.id
                            ? { border: "1px solid #334155", background: "#131f35" }
                            : {}),
                        }}
                      >
                        <div style={styles.feedTop}>
                          ...
                          {policy.category ? <CategoryBadge category={policy.category} /> : null}
                        </div>
                        ...
                        <div style={styles.cardList}>
                          {personCoaching.map((item) => (
                            <div key={item.id} style={styles.feedCard}>
                              ...
                              {item.leadership_notes ? (
                                <div style={{ ...styles.feedSection, borderLeft: "3px solid #2563eb", paddingLeft: "12px", marginTop: "12px" }}>
                                  <div style={{ ...styles.feedLabel, color: "#60a5fa" }}>Guidance</div>
                                  <div style={styles.feedBody}>{item.leadership_notes}</div>
                                </div>
                              ) : null}
                            </div>
